...
async function handleUnlock() {
    ...
    if (check) {
        ...
        if (decrypt(check.value, enteredPassword) === 'ok') {
            ...
            await runMigrationV2(); // Run migration after authentication
            startApp();
        } else {
        ...
        password = enteredPassword;
        isAuthenticated = true;
        await runMigrationV2(); // Also run for new users to set the flag
        startApp();
    }
}
...
async function runMigrationV2() {
    ...
    console.log("Running one-time migration to v2 folder system...");
    ...
    for (const encryptedTask of encryptedTasks) {
        ...
        if (task) {
            const fsNode = {
                ...
                type: "task",
                name: task.title,
                order: task.id, // Tasks don't have 'order' field, use ID
                itemRefId: task.id
            };
            const encryptedNode = encrypt(fsNode, password);
            await db.put('fs_nodes', { id: fsNode.id, data: encryptedNode });
...
async function renderNotesList() {
    ...
    notesEmptyState.classList.add('hidden');
                ...
                folders.forEach(folder => {
                    ...
                    folderEl.dataset.type = 'folder'; // For drag-drop
    ...
    for (const noteNode of items) {
        ...
        noteEl.dataset.id = noteNode.id; // Use fs_node id for drag/drop
        noteEl.dataset.type = 'note'; // For drag-drop
...
async function deleteNote(nodeId) {
    ...
    // Delete from notes store
    await db.delete('notes', Number(node.itemRefId));

    // Delete from fs_nodes store
...
async function renderTasksList() {
    ...
    for (const taskNode of items) {
        ...
        taskEl.dataset.id = taskNode.id; // Use fs_node id
...
function setupDragAndDrop() {
    ...
    const handleDragOver = (e) => {
        ...
        // Autoscroll logic
        const scrollThreshold = 80; // 80px from the edge of the viewport
    ...
    const handleDrop = async (e) => {
        ...
        if (folderTarget && folderTarget !== draggedItem) {
            ...
            if (encryptedNode) {
                ...
                draggedItem.remove(); // Visually move it
            }
        } else {
            const afterElement = getDragAfterElement(container, e.clientY);
            if (afterElement) {
